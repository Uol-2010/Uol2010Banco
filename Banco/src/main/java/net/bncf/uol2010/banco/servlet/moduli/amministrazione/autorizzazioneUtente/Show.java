 ...
 * 
 */
package net.bncf.uol2010.banco.servlet.moduli.amministrazione.autorizzazioneUtente;
...
public class Show
{
	 ...
	 * @throws IOException
	 */
	public MessageElement show(MxMultipartRequest request, MessageElement show, String id) throws ServletException, IOException
	{
		MessageElement risultati = null;

		risultati = new MessageElement();
		risultati.setName("risultati");
		return show(request, show, id, risultati);
	}
	 ...
	 * @throws IOException
	 */
	public MessageElement show(MxMultipartRequest request, MessageElement show, String id, MessageElement risultati) throws ServletException, IOException
	{
		...
		try
		{
			if (id != null && !id.trim().equals(""))
				...
				table = new AutorizzazioniUte(Configuration.poolUol2010, request.getParameterMap());
				
			if (id != null && 
					!id.trim().equals(""))
				table.setCampoValue("idAutorizzazioniUtente", id);
			else
			{
  			if (request.getParameter("Ric_idAutorizzazioniUtente") != null && 
  					!request.getParameter("Ric_idAutorizzazioniUtente").equals(""))
  				table.setCampoValue("idAutorizzazioniUtente", request.getParameter("Ric_idAutorizzazioniUtente"));
			...
			{
				show.addChildElement(table.viewNavigatore(datiXml.getConvert(), datiXml.getConvertUri(), "AutUte", "show"));
	...
	private void show(ResultSet rsServizi, MessageElement risultato) throws SOAPException, SQLException
	{
		...
		try
		{
			datiXml.getConvert().addChildElement(risultato, "idAutorizzazioniUtente", rsServizi.getString("idAutorizzazioniUtente"), true);
			datiXml.getConvert().addChildElement(risultato, "descrizione", rsServizi.getString("descrizione"), true);
			datiXml.getConvert().addChildElement(risultato, "autorizzazioneDef", rsServizi.getString("autorizzazioneDef"), false);
		}
		catch (SOAPException e)
		{
			throw e;
