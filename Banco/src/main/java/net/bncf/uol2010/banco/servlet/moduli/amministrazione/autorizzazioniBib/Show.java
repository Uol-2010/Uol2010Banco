	...
	public MessageElement show(MxMultipartRequest request, MessageElement show, String id, MessageElement risultati, String modulo) throws ServletException, IOException
	{
		...
		try
		{
			if (id != null && !id.trim().equals(""))
				...
				table = new AutorizzazioniBib(Configuration.poolUol2010, request.getParameterMap());
				
			if (id != null && 
					!id.trim().equals(""))
				table.setCampoValue("idAutorizzazioniBib", id);
			else
			{
  			if (request.getParameter("Ric_idAutorizzazioniBib") != null && 
  					!request.getParameter("Ric_idAutorizzazioniBib").equals(""))
  				table.setCampoValue("idAutorizzazioniBib", request.getParameter("Ric_idAutorizzazioniBib"));
