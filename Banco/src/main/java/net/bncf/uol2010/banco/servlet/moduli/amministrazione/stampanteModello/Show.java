 ...
 * 
 */
package net.bncf.uol2010.banco.servlet.moduli.amministrazione.stampanteModello;
...
public class Show
{
	...
	public MessageElement show(MxMultipartRequest request, MessageElement show, String id, MessageElement risultati, String modulo) throws ServletException, IOException
	{
		...
		try
		{
			if (id != null && !id.trim().equals(""))
				...
				table = new StampanteModello(Configuration.poolUol2010, request.getParameterMap());
				
			if (id != null && 
					!id.trim().equals(""))
				table.setCampoValue("idStampanteModello", id);
			else
			{
  			if (request.getParameter("Ric_idStampanteModello") != null && 
  					!request.getParameter("Ric_idStampanteModello").equals(""))
  				table.setCampoValue("idStampanteModello", request.getParameter("Ric_idStampanteModello"));
	...
	private void show(ResultSet rsServizi, MessageElement risultato) throws SOAPException, SQLException
	{
		...
		try
		{
			datiXml.getConvert().addChildElement(risultato, "idStampanteModello", rsServizi.getString("idStampanteModello"), true);
			datiXml.getConvert().addChildElement(risultato, "descrizione", rsServizi.getString("descrizione"), true);
			datiXml.getConvert().addChildElement(risultato, "fileModello", rsServizi.getString("fileModello"), false);
		}
		catch (SOAPException e)
		{
			throw e;
